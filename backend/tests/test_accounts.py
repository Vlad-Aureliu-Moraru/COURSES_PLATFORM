...
from apps.accounts.models import PendingSignup, PasswordResetToken, SignupCode, User
...
def signup_and_get_code(client, email='test@example.com'):
    ...
    return get_otp_code(PendingSignup.objects.get(email=email))


def test_signup_creates_pending_signup_sends_otp_no_user(db, mailoutbox):
    client = APIClient()
    ...
    pending = PendingSignup.objects.get(email='test@example.com')
    assert pending.password_hash
    assert pending.first_name == 'Test'
    assert SignupCode.objects.filter(pending=pending).count() == 1
...
def test_signup_verify_creates_user_returns_tokens(db, mailoutbox):
    client = APIClient()
    code = signup_and_get_code(client)
    ...
    assert not PendingSignup.objects.filter(email='test@example.com').exists()
...
def test_signup_verify_expired_code_rejected(db):
    ...
    client.post('/api/v1/auth/signup/', signup_payload(), format='json')
    pending = PendingSignup.objects.get(email='test@example.com')
    SignupCode.objects.filter(pending=pending).update(
        expires_at=timezone.now() - timedelta(minutes=1)
    )
    code = get_otp_code(pending)
...
def test_signup_resend_adds_code_without_invalidating_first(db, mailoutbox):
    client = APIClient()
    client.post('/api/v1/auth/signup/', signup_payload(), format='json')
    pending = PendingSignup.objects.get(email='test@example.com')
    first_code = get_otp_code(pending)
    first_count = SignupCode.objects.filter(pending=pending).count()

    resend = client.post(
        '/api/v1/auth/signup/resend/', {'email': 'test@example.com'}, format='json'
    )
    assert resend.status_code == 200
    assert SignupCode.objects.filter(pending=pending).count() == first_count + 1
    assert len(mailoutbox) == 2  # original otp + resent

    # The original code must still verify after a resend (regression test).
    verify = client.post(
        '/api/v1/auth/signup/verify/',
        {'email': 'test@example.com', 'code': first_code},
        format='json',
    )
    assert verify.status_code == 200
